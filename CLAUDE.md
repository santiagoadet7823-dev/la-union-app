...
Hay varios números que conviven. **1.13.7** es OTA-solo (JS): `APP_VERSION` y el bundle van en 1.13.7; el APK sigue en 1.13.0 (agosto 2026). ⚠️ 1.13.0 es un cambio NATIVO (el ancla del uploader): sale por APK, y hay que publicar la misma versión como OTA para los que ya lo tienen.
...
|---|---|---|---|
| `APP_VERSION` | [src/version.js](src/version.js) | `1.13.7` | Se compara con `app_config.latest_version`; se reporta en `estado_dispositivo.app_version` |
...
| `app_config.bundle_version` + `latest_version` | Supabase | `1.13.7` ✅ publicado | Qué bundle OTA deben bajar los teléfonos |
