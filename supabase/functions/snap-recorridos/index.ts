...
// Cada motor acierta en su modo y delira en el otro, y la guarda anti-detour (×2,5) rechaza los dos
// casos malos. O sea: el modo elige el motor, y la guarda es la red por si el modo se equivocó.
//
...
import {
  type P, GAP_MS, cap, firmaRun, isStationary, ms, pathLenLL, segLenP, splitGaps, splitModo, thin,
} from './segmentar.ts'
...
const ALGO = 8           // versión del algoritmo; sube al cambiar la lógica → invalida el cache viejo
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const crudos = { corto: 0, largoPeaton: 0, creciendo: 0, presupuesto: 0, osrm: 0, detour: 0 }
    ...
    for (const [id, pts] of Object.entries(byUser)) {
      ...
      for (const seg of splitGaps(pts)) {
        ...
        for (const run of splitModo(seg)) {
          ...
          if (lenM < MIN_RUN_M) { crudos.corto++; piezas.push({ f, g: crudo }); continue }
          if (!run.auto && lenM > FOOT_LEN) { crudos.largoPeaton++; piezas.push({ f, g: crudo }); continue }
          if (creciendo) { crudos.creciendo++; piezas.push({ f, g: crudo }); continue }
          if (ruteos >= MAX_RUTEOS) { truncados++; crudos.presupuesto++; piezas.push({ f, g: crudo }); continue }
          ...
          if (!g) { osrmMiss = true; crudos.osrm++; piezas.push({ f, g: crudo }); continue }
          ...
          const ok = pathLenLL(g) <= MAX_DETOUR * lenM + 50
          if (!ok) crudos.detour++
          piezas.push({ f, g: ok ? g : crudo })
        }
      }
    ...
    return json({ recorridos, ruteos, truncados, autos, pies, crudos })
  } catch (e) {
